...
class User
{
    ...
    public static function findByEmail($email)
    {
        ...
        $stmt->execute([$email]);
        return $stmt->fetch(); // array hoặc false
    }
    ...
    {
        $conn = Database::getConnection();
        $stmt = $conn->query("SELECT COUNT(*) AS c FROM nguoi_dung");
        $row = $stmt->fetch();
        return (int)$row['c'];
    }
    ...
    {
        $conn = Database::getConnection();
        $stmt = $conn->query("SELECT COUNT(*) AS c FROM nguoi_dung WHERE trang_thai_hoat_dong = 0");
        $row = $stmt->fetch();
        return (int)$row['c'];
    }
    ...
    {
        $conn = Database::getConnection();
        $stmt = $conn->query("SELECT * FROM nguoi_dung ORDER BY ma_nguoi_dung DESC");
        return $stmt->fetchAll();
    }

    // Cập nhật trạng thái hoạt động (active / khóa)
    ...
    // Lấy danh sách email ứng viên đang active & đồng ý nhận mail
